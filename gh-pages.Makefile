...
GHP_SRC?=.
GHP_DEST?=.
GHP_CACHE_DIR?=$(HOME)/.gh-pages-cache
GHP_BRANCH?=gh-pages
GHP_REMOTE?=origin
GHP_DEPTH?=1
GHP_RM?=.
...
ifeq ($(OS),Windows_NT)
	...
	$(GCD) ls-remote --exit-code . $(GHP_REMOTE)/$(GHP_BRANCH) && ($(GIT) checkout -b $(GHP_BRANCH) --track $(GHP_REMOTE)/$(GHP_BRANCH); $(GIT) clean; $(GIT) reset --hard $(GHP_REMOTE)/$(GHP_BRANCH)) || $(GIT) checkout --orphan $(GHP_BRANCH)
endif
...
ifeq ($(OS),Windows_NT)
	...
	cd $(GHP_SRC) ; find . -type d -exec mkdir -p "$(GHP_CACHE_DIR)/$(GHP_DEST)/{}" \; -o -type f -exec cp '{}' "$(GHP_CACHE_DIR)/$(GHP_DEST)/{}" \;
endif
